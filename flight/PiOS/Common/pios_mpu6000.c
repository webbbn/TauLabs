...
/* Private constants */
#define MPU6000_TASK_PRIORITY	(tskIDLE_PRIORITY + configMAX_PRIORITIES - 1)	// max priority
#define MPU6000_TASK_STACK		(484 / 4)
...
int32_t PIOS_MPU6000_Init(uint32_t spi_id, uint32_t slave_num, const struct pios_mpu60x0_cfg *cfg)
{
	...
	int result = xTaskCreate(PIOS_MPU6000_Task, (const signed char *)"pios_mpu6000",
	                         MPU6000_TASK_STACK, NULL, MPU6000_TASK_PRIORITY,
	                         &pios_mpu6000_dev->TaskHandle);
...
static void PIOS_MPU6000_Task(void *parameters)
{
	while (1) {
		...
		xQueueSendToBack(pios_mpu6000_dev->accel_queue, (void *)&accel_data, 0);

		xQueueSendToBack(pios_mpu6000_dev->gyro_queue, (void *)&gyro_data, 0);
